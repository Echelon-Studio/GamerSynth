...
(function(){
	...
	XBOX MAPPINGS:
		...
		Axii:
			0 left stick horizontal (left is negative)
			1 left stick vertical (up is negative)
			2 left trigger (neutral/resting is negative)
			3 right stick horizontal (left is negative)
			4 right stick vertical (up is negative)
			5 right trigger (neutral/resting is negative)
			6 d-pad horizontal (left is negative)
			7 d-pad vertical (up is negative)
	*/

	GamepadWrapper = function() {
		...
		if (this.supported) {
			...
			function create_controls(mapScheme){
				...
				for (var i in map.buttons) {
					...
					this.controlListeners[control.name] = control;
				}
			...
			*/
			...
			function report_listeners() {
				...
				*/
				...
				if (this.gamepad) {
					...
					var mapNames = MAPPING_NAMES[this.mapScheme];
					if (mapNames) {
						var map = MAPPINGS[this.mapScheme];

						for (var j in mapNames.buttons) {
							var controlName = mapNames.buttons[j];
							var control = map.buttons[controlName];
							control.updateListeners(this.gamepad.buttons[j].value);
						...
						for (var j in mapNames.axes) {
							...
							control.updateListeners(this.gamepad.axes[j]);
			...
			window.addEventListener("gamepadconnected", function(e) {
				...
				this.mapScheme = checkGamepadMapping(e.gamepad);
			...
			function setupChromeListener(){
	            chrome_gamepad_listener_interval = window.setInterval(function() {
	                if(navigator.getGamepads()[0]) {
	                    if(!this.gamepad_connected) $(window).trigger("gamepadconnected", {gamepad: navigator.getGamepads()[0]});
